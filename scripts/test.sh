...
if [ $# -eq 2 ]; then
    ...
    for x in 5; do
        y=4
        docker cp "$(dirname "$0")/../backups/cassandra-n$y/" "cassandra-n$x":"/var/lib/cassandra/data/data/"
        for dir in $(find "$(dirname "$0")/../backups/cassandra-n$y/db3_course_project/" -mindepth 1 -maxdepth 1 -type d); do
            dir=${dir%*/}
            dir=${dir##*/}
